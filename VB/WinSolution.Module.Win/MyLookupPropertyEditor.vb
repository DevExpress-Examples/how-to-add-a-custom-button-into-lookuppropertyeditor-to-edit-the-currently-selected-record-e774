...
Namespace WinSolution.Module.Win
	Public Class MyLookupPropertyEditor
		...
		Protected Overrides Sub SetupRepositoryItem(ByVal item As RepositoryItem)
            ...
            CType(item, DevExpress.ExpressApp.Win.Editors.RepositoryItemLookupEdit).Init(DisplayFormat, Me.Helper)
			Dim appearance As New AppearanceObject()
			appearance.Font = New Font(FontFamily.GenericSansSerif, 10F, FontStyle.Underline)
            CType(item, DevExpress.ExpressApp.Win.Editors.RepositoryItemLookupEdit).Buttons.Add(New EditorButton(ButtonPredefines.Glyph, "Edit", -1, True, True, True, HorzAlignment.Default, Nothing, New KeyShortcut(System.Windows.Forms.Keys.Enter), appearance, "Press this button to edit the object"))
            AddHandler (CType(item, DevExpress.ExpressApp.Win.Editors.RepositoryItemLookupEdit)).ButtonClick, AddressOf MyLookupPropertyEditor_ButtonClick
		End Sub
		Private Sub MyLookupPropertyEditor_ButtonClick(ByVal sender As Object, ByVal e As ButtonPressedEventArgs)
			If (Not e.Button.IsDefaultButton) Then
				Dim objectSpace As ObjectSpace = Helper.Application.CreateObjectSpace()
